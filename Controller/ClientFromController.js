...
exports.getRegistrationById = async (req, res) => {
  try {
    const registration = await Registration.findById(req.params.id).populate("officeConfirmation.country").populate("officeConfirmation.work") ;
    if (!registration) {
      return res.status(404).json({ message: 'Registration not found' });
    }
...
exports.getRegistrationByLeadId = async (req, res) => {
  try {
    const registration = await Registration.findOne({ leadId: req.params.leadId }).populate("officeConfirmation.country").populate("officeConfirmation.work") ;
...
exports.ApplyInterview = async (req, res) => {
  ...
  try {
    ...
    const registrations = await Registration.findOne({ leadId: leadId });
    if (!registrations) return res.status(404).json({ message: "Lead not found" });

    if (!registrations.passportNumber) {
      return res.status(400).json({ message: "Lead must have a passport number" });
    }
    if (registrations.InterviewStatus != 'Not Applied') {
      return res.status(400).json({ message: "Already Applied For Interview" });
    }
    const updatedLead = await Registration.findOne({ leadId: leadId });

    updatedLead.InterviewManager = interviewManagerId,
      updatedLead.InterviewAppliedBy = callingTeamId,
      updatedLead.InterviewApplyDate = new Date(),
      updatedLead.InterviewStatus = "Applied",
    ...
    console.error("ApplyForInterview Error:", error);
    res.status(500).json({ message: "Internal server error", error: error.message });
  }
}
...
exports.markInterview = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const contact = await Registration.findByIdAndUpdate(
      id,
    ...
    res.json({ message: `Interview marked as ${status}`, contact });
  } catch (error) {
    res.status(500).json({
